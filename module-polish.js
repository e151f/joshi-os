(()=>{
  ...
  function style(){if(document.getElementById('module-polish-css'))return;const s=document.createElement('style');s.id='module-polish-css';s.textContent=`
    /* Compact Glow Up-style module imagery. Legacy hero banners are disabled. */
    ...
    /* Wallet composition: image/progress, then journal/history, then metrics. */
    .wallet-hero-grid,.wallet-main-grid{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,.85fr);gap:12px}
    .wallet-visual{padding:0!important;overflow:hidden}
    .wallet-visual img{display:block;width:100%;height:260px;object-fit:cover}
    .wallet-visual .module-visual-caption{padding:12px 16px}
    .wallet-progress{min-height:100%;display:flex;flex-direction:column;justify-content:center;padding:22px}
    .wallet-progress p{color:var(--muted);margin:12px 0 4px}
    .wallet-progress-bar{height:8px;border-radius:999px;background:#241b23;overflow:hidden;margin:14px 0}
    .wallet-progress-bar i{display:block;height:100%;background:var(--pink);transition:width .2s}
    .wallet-metrics{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px}
    @media(max-width:700px){.module-visual img{height:175px}.wallet-hero-grid,.wallet-main-grid,.wallet-metrics{grid-template-columns:1fr}.wallet-visual img{height:220px}}
  ...
  function addVisual(){const v=document.querySelector('#view'),page=v?.dataset.page;if(!v||!heroes[page]||page==='glow'||page==='wallet')return;cleanupLegacy();if(v.querySelector('.module-visual'))return;const [img,title]=heroes[page],section=v.querySelector('section')||v.firstElementChild;if(!section)return;const head=section.querySelector('.viewhead');const h=document.createElement('div');h.className='module-visual';h.innerHTML=`<img src="${img}" alt="${title}"><div class="module-visual-caption"><span>MODULE</span><small>${title}</small></div>`;if(head)head.insertAdjacentElement('afterend',h);else section.insertBefore(h,section.firstChild)}
