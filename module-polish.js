(()=>{
  ...
  function style(){if(document.getElementById('module-polish-css'))return;const s=document.createElement('style');s.id='module-polish-css';s.textContent=`
    ...
    /* Wallet: match Glow Up's 1.15 / .85 two-column image + progress composition. */
    #view[data-page="wallet"] .module-visual{width:auto;margin:0 0 14px;grid-column:1 / 2}
    #view[data-page="wallet"] .module-visual img{height:260px}
    #view[data-page="wallet"] .module-visual-caption{padding:12px 16px}
    #view[data-page="wallet"] > section > .viewhead + .module-visual + .enh-grid{grid-template-columns:minmax(0,1.15fr) minmax(0,.85fr)!important}
    ...
    @media(max-width:700px){.module-visual img{height:175px}#view[data-page="wallet"] .module-visual img{height:220px}#view[data-page="wallet"] > section > .viewhead + .module-visual + .enh-grid{grid-template-columns:1fr!important}.memory-hero{padding:18px;align-items:flex-start}.memory-count{text-align:left}.memory-stats{grid-template-columns:repeat(2,1fr)}.memory-analysis{grid-template-columns:1fr}}
